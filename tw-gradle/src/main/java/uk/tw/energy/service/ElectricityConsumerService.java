...
public class ElectricityConsumerService {
    ...
    public ElectricityConsumerService(AccountService accountService, MeterReadingService meterReadingService, PricePlanService pricePlanService) {
    ...
    public Optional<BigDecimal> calculateUsageCostLastWeek(String smartMeterId) {
        ...
        List<ElectricityReading> electricityReadingList = electricityReadings.get().stream()
                .filter(electricityReading -> electricityReading.time().isAfter(oneWeekAgo)).toList();
        ...
        BigDecimal averageReading = totalReading.divide(BigDecimal.valueOf(electricityReadingList.size()), RoundingMode.HALF_UP);
        ...
        Map<String, BigDecimal> tariffUnitPricesPlan = pricePlanService.getConsumptionCostOfElectricityReadingsForEachPricePlan(smartMeterId)
